...
interface MedicoImportData extends MedicoExcelRow {
  rowNumber: number;
  status: 'pending' | 'success' | 'error';
  error?: string;
}

interface ExcelImportMedicosProps {
  isOpen: boolean;
  onClose: () => void;
  onImportComplete: () => void;
}

const ExcelImportMedicos: React.FC<ExcelImportMedicosProps> = ({
  isOpen,
  onClose,
  onImportComplete
}) => {
  ...
  const readExcelFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const mappedData: MedicoImportData[] = jsonData.map((row, index) => {
          const nome = findValue(row, 'nome', 'NOME', 'Nome', 'name', 'NAME', 'Name');
          const cns = findValue(row, 'cns', 'CNS', 'Cns', 'crm', 'CRM', 'Crm');
          const especialidade = findValue(row, 'especialidade', 'ESPECIALIDADE', 'Especialidade', 'specialty', 'SPECIALTY');
          const id = findValue(row, 'id', 'ID', 'Id', 'hospital_id', 'HOSPITAL_ID', 'Hospital_Id', 'hospitalId');
          
          console.log(`📝 Linha ${index + 2}:`, { nome, cns, especialidade, id });
          ...
          return {
            ...
            especialidade,
            id,
            rowNumber: index + 2, // +2 porque começamos na linha 2 (1 é header)
            status: 'pending'
          };
        });
        ...
        const hasData = mappedData.some(row => 
          row.nome || row.cns || row.especialidade || row.id
        );
  ...
  const validateRow = (row: MedicoImportData): string | null => {
    if (!row.nome || row.nome.trim() === '') {
      return 'Nome é obrigatório';
    }
    if (!row.cns || row.cns.trim() === '') {
      return 'CNS é obrigatório';
    }
    if (!row.especialidade || row.especialidade.trim() === '') {
      return 'Especialidade é obrigatória';
    }
    if (!row.id || row.id.trim() === '') {
      return 'ID do hospital é obrigatório';
    }
    return null;
  };
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < updatedData.length; i++) {
      ...
      try {
        // Importar o médico usando a API do Supabase
        const { supabase } = await import('../services/supabase');
        
        const medicoData = {
          nome: row.nome.trim(),
          especialidade: row.especialidade.trim(),
          crm: row.cns.trim(), // Usando CNS no campo CRM
          telefone: '', // Campos vazios por padrão
          email: '',
          hospital_id: row.id.trim()
        };

        console.log(`📝 Importando médico ${i + 1}/${updatedData.length}:`, medicoData);
        ...
        row.status = 'success';
        successCount++;
        console.log(`✅ Médico importado com sucesso:`, data);
      } catch (error: any) {
        console.error(`❌ Erro ao importar médico da linha ${row.rowNumber}:`, error);
        row.status = 'error';
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Importar Médicos via Excel"
      size="large"
    >
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ul className="text-xs text-blue-800 list-disc list-inside space-y-1">
            <li><strong>nome</strong>: Nome completo do médico</li>
            <li><strong>cns</strong>: CNS do médico (será usado no campo CRM)</li>
            <li><strong>especialidade</strong>: Especialidade médica</li>
            <li><strong>id</strong>: ID do hospital (obtido do banco de dados)</li>
          </ul>
          <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
            ...
            <ul className="text-xs text-yellow-800 list-disc list-inside mt-1 space-y-1">
              <li>Os nomes das colunas podem ter maiúsculas ou minúsculas</li>
              <li>Espaços extras serão removidos automaticamente</li>
              <li>Certifique-se de que não há linhas vazias antes do cabeçalho</li>
              <li>Formato recomendado das células: "Geral" ou "Texto"</li>
            </ul>
          </div>
          <p className="text-xs text-blue-800 mt-2">
            ⚠️ <strong>Importante:</strong> Médicos com mesmo nome/CNS podem ser duplicados para diferentes hospitais.
          </p>
        </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-gray-900">
                Preview dos Dados ({previewData.length} médicos)
              </h4>
              ...
              <table className="w-full text-xs">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left font-medium text-gray-600">CNS</th>
                    <th className="px-3 py-2 text-left font-medium text-gray-600">Especialidade</th>
                    <th className="px-3 py-2 text-left font-medium text-gray-600">Hospital ID</th>
                ...
                <tbody>
                  {previewData.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">{row.cns}</td>
                      <td className="px-3 py-2">{row.especialidade}</td>
            ...
            >
              {importing ? (
                ...
                <>
                  ...
                  Importar {previewData.length} Médico(s)
                </>
              )}
            </Button>
          )}
...
export default ExcelImportMedicos;
